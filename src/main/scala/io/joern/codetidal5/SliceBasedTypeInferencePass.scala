...
import io.joern.codetidal5.Main.JoernTIConfig
import io.joern.dataflowengineoss.slicing.{ObjectUsageSlice, ProgramUsageSlice, UsageSlicing, UsagesConfig}
...
class SliceBasedTypeInferencePass(
  ...
  config: JoernTIConfig = JoernTIConfig()
) extends CpgPass(cpg) {

  private lazy val logger = LoggerFactory.getLogger(classOf[SliceBasedTypeInferencePass])
  private lazy val sliceConfig =
    UsagesConfig(minNumCalls = config.minNumCalls, excludeOperatorCalls = config.excludeOperatorCalls)
  ...
  private def builtinTypes(res: InferenceResult): InferenceResult =
    res.typ match {
      case x if GlobalBuiltins.builtins.contains(x.capitalize) => res.copy(typ = s"$ecmaPrefix${x.capitalize}")
      case _                                                   => res
    }
